...
import { Users, HatGlasses, Mail, Lock } from "lucide-react"
...
const Home = () =>{
  ...
  return(
    <div id="top" className='scroll-mt-24'>
      ...
      <section id="About Us" className='mt-10 w-screen md:mt-15 lg:mt-0 scroll-mt-30 mb-15 '>
        ...
        <div className='flex flex-col gap-5'>
          <div ref={sliderRef1} className='flex self-start'>
            <Slider pic={HatGlasses} name="Built for Anonymity" description="There are no profile pictures, usernames are randomly generated, and we don’t ask for personal details. You can share without worrying about being recognized or traced." pos="start" bgColor="dark" isVisible={isVisible1} duration="1200"/>
          </div>
          <div ref={sliderRef2} className='flex self-end'>
            <Slider pic={Lock} name="Secure Content" description="Your posts, comments, and drafts stay secure and are read only by the people it’s meant for, no outsiders or advertisers." pos="end" bgColor="light" isVisible={isVisible2} duration="4200"/>
          </div>
          <div ref={sliderRef3} className='flex self-start'>
            <Slider pic={Users} name="Human-Led Moderation" description="All moderation is handled by real people, not third-party AI systems. Your data isn’t sent off to be analyzed or trained elsewhere, it stays here." pos="start" bgColor="dark" isVisible={isVisible3} duration="7200"/>
          </div>
          <div ref={sliderRef4} className='flex self-end'>
            <Slider pic={Mail} name="Contact Us" description="For any enquires or issues contact us at this email address: p2pTeam@gmail.com" pos="end" bgColor="light" isVisible={isVisible4} duration="10200"/>
          </div>
        </div>
      </section>

      <section className='w-screen md:mt-10 lg:mt-0 scroll-mt-30'>
        <p className="w-screen bg-[var(--color-six)] text-slate-100 font-semibold px-5 py-5 text-sm text-center bg-opacity-80">
          <button
            className='cursor-pointer border-none underline underline-offset-4 decoration-1 decoration-transparent hover:decoration-current hover:text-white transition-[text-decoration-color] duration-500'
      ...
      {openTerms && <TermsAndConditions open={openTerms} onOpenChange={setOpenTerms} />}
  </div>
